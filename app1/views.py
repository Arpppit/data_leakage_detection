...
def my_handler(sender, instance, created, **kwargs): 
	...
	post_save.connect(my_handler, sender=Users.objects.get(username = 'arppit'))
...
def modify_file(filename, clientid, n):
    ...
    #cipher embedding
    print(cipher)
    encryption_suite = AES.new('thisisorignalkey', AES.MODE_CBC, 'thisisinitvector')
#c = pad(c.encode(), AES.block_size)
    plain = encryption_suite.encrypt(cipher)
    ...
    c.drawImage(f"{folder}/media/documents/image_small_hash.png", 0, 0, preserveAspectRatio=True)
    c.save()
    filename = t
    ...
    watermark = PdfFileReader(open(f"{folder}/media/documents/watermark.pdf", "r+b"))
    ...
    outputStream = open(f"{folder}/media/documents/document-output-{n}.pdf", "w+b")
...
def extraction():
	...
	name = str(q.document)
	folder = os.getcwd()
	document_location = f"{folder}/media/"
	file_loc = document_location + name
	logo_loc = document_location + "detector/logo"
	...
	im = cv2.imread(document_location + 'detector/logo-001-000.png')
	cipher = []
	print(im.shape)
	for i in range(0,24):
	...
	cipher_text = decryption_suite.decrypt(base64.b64decode(cipher)).decode('utf-8')
...
def deletefile(request):
	...
	global logs
	document_location = '/'.join(os.path.dirname(__file__).split('/')[:-1]) + '/media/' 
	if request.method == 'POST':
		if request.POST.get('filename'): #filename is name attribute of the button clicked in template
			name = request.POST.get('filename')
